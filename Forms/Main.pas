...
begin
   //провер€ем наличие задач в стеке
...
begin
  ...
  if MessageDlg('”далить задачу?' ,mtWarning, [mbYes, mbNo], 0) = mrYes then
    begin
      DM.dbSetting.Delete;
    end;
...
begin
  ...
  begin
    ...
    DM.addTask.Parameters.ParamByName('LastStart').Value := 'Ќикогда' ;
...
begin
  ...
  begin
    LogZip(DM.pathExe + 'Logs',
           ...
           '«адача <' + DM.qStartTaskNameTask.AsString + '> - путь архивации не найден, задача выключена' );
  ...
  //после выполнени€ задачи определ€ем следующий старт задачи
...
var
  ...
  BM: TBookmark;
begin
  //обновл€ем грид с задачами
  DM.dbSettingRefresh;

  //перебираем задачи попадающие в запрос дл€ стека
  ...
  begin
    //добавл€ем в стек задачу

    flFind := ExistRecStack();
    //провер€ем наличие в стеке задач
    if flFind then
    begin
      // провер€ем на повторый запуск                     !!!!!!!!!!!!!!!!!!
    ...
    DM.addExecTask.Parameters.ParamByName('onExec').Value := 1 ;   // статус ¬ ожидании...
